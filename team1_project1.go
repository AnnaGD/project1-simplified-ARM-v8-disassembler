...
func getInstructionFormat(content string) string {
	...
	for fileScanner.Scan() {
		...
		if opcode == 2038 {
			...
			s := fmt.Sprintf(jar[0:0], jar[1:6], jar[7:11], jar[12:16], jar[17:21], jar[22:26], jar[27:31], Answer)
			result += s
			result += "\n"
			Holder = ""
			jar = ""
		...
		case 1:
			...
			if opcode >= 1684 && opcode <= 1687 || opcode >= 1940 && opcode <= 1943 {
				...
				s := fmt.Sprintf(jar[0:11]+" "+jar[12:16]+" "+jar[17:22]+" "+jar[23:27]+" "+jar[28:32], Counter, Answer, register1)
			...
			if opcode >= 1160 && opcode <= 1161 || opcode >= 1672 && opcode <= 1673 {
				...
				s := fmt.Sprintf(jar[0:11]+" "+jar[12:16]+" "+jar[17:22]+" "+jar[23:27]+" "+jar[28:32], Counter, Answer, register1, register2)
			...
			if opcode == 1104 || opcode == 1112 || opcode == 1360 || opcode == 1624 ||
				...
				//printing directly to file function takes %s "string" and &d digit value
				s := fmt.Sprintf(jar[0:11], jar[12:16], jar[17:22], jar[23:27], jar[28:32], Counter, Answer, register1, register2, register3)
				//adding new string and linebreak
				result += s
				result += "\n"
				Holder = ""
